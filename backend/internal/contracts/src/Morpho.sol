...
// We would like to acknowledge that our market is a Forked from morpho.
// However, we don't intend to use their code in production, we only forked it so that we can use it as a Proof of Concept for our project
//  for the purposes of the hackathon since developing one from scratch would be a huge undertaking in such a short timeframe.
// If this project is successful, we will develop our markets from scratch.
contract Morpho is IMorphoStaticTyping {
    ...
    MarketParams public DEFAULT_MARKET_PARAMS = MarketParams({
        ...
        irm: 0x2EF5fbE0552e1905cAEDdbeDdf27524723FbeE97,
    ...
    function setFee(uint256 newFee) external onlyOwner {
        MarketParams memory marketParams = DEFAULT_MARKET_PARAMS;
    ...
    /// @inheritdoc IMorphoBase
    function supply(uint256 assets, uint256 shares, address onBehalf, bytes calldata data)
        external
    ...
    /// @inheritdoc IMorphoBase
    function repay(uint256 assets, uint256 shares, address onBehalf, bytes calldata data)
        external
        returns (uint256, uint256)
    {
        MarketParams memory marketParams = DEFAULT_MARKET_PARAMS;
    ...
    /// @inheritdoc IMorphoBase
    function supplyCollateral(uint256 assets, address onBehalf, bytes calldata data) external {
        MarketParams memory marketParams = DEFAULT_MARKET_PARAMS;
        Id id = marketParams.id();
        require(market[id].lastUpdate != 0, ErrorsLib.MARKET_NOT_CREATED);
    ...
    /// @inheritdoc IMorphoBase
    function liquidate(address borrower, uint256 seizedAssets, uint256 repaidShares, bytes calldata data)
        external
        returns (uint256, uint256)
    {
        MarketParams memory marketParams = DEFAULT_MARKET_PARAMS;
